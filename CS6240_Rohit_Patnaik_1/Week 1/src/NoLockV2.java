...
class NoLockThreadV2 extends Thread{
	...
	public NoLockThreadV2(List<String> res){
		this.res = res;
	}
	
	public NoLockThreadV2(){
		...
		for(int i=0; i<res.size(); i++) {
			...
			if(splitArr[2].equals("TMAX")) {
				if(!hm.containsKey(splitArr[0])){
					...
					val += Integer.parseInt(splitArr[3])+c;
...
public class NoLockV2 extends Loader{
	...
	public static void main(String[] args) {
		...
		NoLockThreadV2 td1 = new NoLockThreadV2(res.subList(0, inputLength/2));
		NoLockThreadV2 td2 = new NoLockThreadV2(res.subList(inputLength/2, inputLength));
		...
		NoLockThreadV2 o = new NoLockThreadV2();
